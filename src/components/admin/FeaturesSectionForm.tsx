...
export const FeaturesSectionForm = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Section fonctionnalités mise à jour !", description: "Les modifications sont en ligne." });
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Badge, titre et sous-titre au-dessus des cartes de fonctionnalités</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div><Label className="text-xs text-muted-foreground">Titre</Label><Input value={form.headline} onChange={(e) => setForm({ ...form, headline: e.target.value })} /></div>
          <div><Label className="text-xs text-muted-foreground">Sous-titre</Label><Input value={form.subheadline} onChange={(e) => setForm({ ...form, subheadline: e.target.value })} /></div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Cartes de fonctionnalités</CardTitle>
          <CardDescription>Éléments individuels affichés en grille</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {form.features.map((f, i) => (
            <div key={i} className="flex items-start gap-2 p-3 rounded-lg border border-border/50">
              <div className="flex-1 space-y-2">
                <Input placeholder="Titre" value={f.title} onChange={(e) => { const features = [...form.features]; features[i] = { ...f, title: e.target.value }; setForm({ ...form, features }); }} />
          ...
          <Button variant="outline" size="sm" onClick={() => setForm({ ...form, features: [...form.features, { title: "", description: "" }] })} className="gap-1"><Plus className="w-3 h-3" /> Ajouter une fonctionnalité</Button>
        </CardContent>
      </Card>
