...
export const HostingPlatformsForm = () => {
  ...
  const handleSave = () => {
    updateSettings.mutate(form, {
      onSuccess: () => toast({ title: "Paramètres d'hébergement enregistrés !" }),
      onError: (e) => toast({ title: "Erreur lors de l'enregistrement", description: e.message, variant: "destructive" }),
    });
  };
  ...
  const addStep = (platformIdx: number) => {
    setForm((prev) => ({
      ...
      platforms: prev.platforms.map((p, i) =>
        i === platformIdx
          ? { ...p, steps: [...p.steps, { title: "Nouvelle étape", description: "", details: [""] }] }
          : p
      ),
    }));
  };
    ...
    setForm((prev) => ({
      ...
      platforms: [
        ...prev.platforms,
        {
          ...
          tagline: "Description de cette plateforme d'hébergement",
          enabled: true,
          color: "bg-primary text-primary-foreground",
          steps: [{ title: "Étape 1", description: "Description de la première étape", details: ["Détail 1"] }],
        },
      ],
    }));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">Plateformes d'hébergement</h2>
          <p className="text-sm text-muted-foreground">Configurez l'assistant d'hébergement présenté aux acheteurs après l'achat</p>
        </div>
      ...
      <Card className="border-accent/30 bg-accent/5">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            />
            <Label className="text-sm">Afficher l'option « Engager un Pro » dans l'assistant d'hébergement</Label>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-xs">Lien de contact</Label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <Label className="text-xs font-semibold">Fonctionnalités incluses</Label>
              <Button
                variant="ghost"
                size="sm"
          ...
          <AccordionItem key={platform.id} value={platform.id} className="border border-border/50 rounded-xl overflow-hidden">
            <AccordionTrigger className="px-4 py-3 hover:no-underline">
              <div className="flex items-center gap-3 flex-1">
                ...
                {!platform.enabled && <Badge variant="secondary" className="text-xs">Désactivé</Badge>}
                <Badge variant="outline" className="text-xs ml-auto mr-2">{platform.steps.length} étape{platform.steps.length > 1 ? "s" : ""}</Badge>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-4 pb-4">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="text-xs">Classe de couleur</Label>
                  ...
                  {platform.steps.map((step, si) => (
                    <Card key={si} className="border-border/30">
                      <CardHeader className="py-3 px-4">
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-sm">Étape {si + 1}</CardTitle>
              ...
              <Label className="text-xs">Titre</Label>
                        ...
                        <div>
                          <Label className="text-xs">Commande (optionnelle)</Label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <Label className="text-xs">URL du lien (optionnelle)</Label>
                          ...
                          <div>
                            ...
                            <Input value={step.link_label || ""} onChange={(e) => updateStep(pi, si, { link_label: e.target.value || undefined })}                             placeholder="Ouvrir la plateforme" />
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <Label className="text-xs">Détails de l'étape</Label>
                ...
                <Button variant="destructive" size="sm" onClick={() => removePlatform(pi)} className="gap-1">
                  <Trash2 className="w-3 h-3" /> Supprimer la plateforme
                </Button>
              </div>
            </AccordionContent>
          </AccordionItem>
      ...
      <Button variant="outline" onClick={addPlatform} className="w-full gap-2">
        <Plus className="w-4 h-4" /> Ajouter une nouvelle plateforme
      </Button>
    </div>
  );
};
